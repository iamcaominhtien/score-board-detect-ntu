...
class DetectTableAPI:
	...
	neural_model_reg = None

	def __init__(self, input_image: np.ndarray):
		self.input_image = input_image
		if DetectTableAPI.neural_model_reg is None:
			DetectTableAPI.neural_model_reg = DetectTableAPI.load_model()

	def __pre_process_image(self):
		"""
		...
		The function then cuts the table in the image into rows by row using the filtered horizontal
		 and vertical lines and returns the cells.

		Returns:
			list: A list of cells in the table. Each cell is represented as a dictionary with keys 'point', 'img', and 'img_bin'.
		"""
		h_raw, _ = self.input_image.shape[:2]
		if h_raw > 1500:
		...
		img_bin = helper.convert_to_bin_image(self.input_image, 5)
		detect_table_tool = DetectTable(img_bin, self.input_image)
		...
		cells = cut_table_row_by_row(hor_lines, ver_lines, self.input_image, img_bin)
	...
	def detect(self):
		...
		cells = self.__pre_process_image()
		...
		predicted_classes = np.argmax(
			self.neural_model_reg.predict(helper.hog_feature_extraction(numbers_predict)), axis=1
		)
