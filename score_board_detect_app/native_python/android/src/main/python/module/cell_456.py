        ...
        while _run < len(_row_whiteness_needed) - 1:
            ...
            for i in range(MAX_LATERAL_KERNEL, w - MAX_LATERAL_KERNEL):
                ...
                for rule in rules:
                    ...
                    punish = rule['punish']
                    window = im_tmp[h_line_mid_row - top_k:h_line_mid_row + bot_k + 1,
                             i - lat_k:i + lat_k + 1]
                    if window.size == 0:
                        window_mean = np.nan
                    else:
...
def remove_vertical_line(im: np.ndarray) -> np.ndarray:
    ...
    _find_vertical_lines = np.where(im_mean >= 0.8 * 255)[0]
...
def cell_456_process(IMG):
    _img = IMG.copy()
    if _img.size == 0 or np.mean(_img) < 100:
        return []
    if _img.ndim > 2:
    ...
    for _idx, _ctr in enumerate(_ctrs_cord):
        ...
        _ctrs, _ = cv2.findContours(frame.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        # sorted by area
        _ctrs = sorted(_ctrs, key=lambda _ctr: cv2.boundingRect(_ctr)[2] * cv2.boundingRect(_ctr)[3], reverse=True)
        for _jdx, _ctr_temp in enumerate(_ctrs):
            if _jdx == 0:
                continue
            x_temp, y_temp, w_temp, h_temp = cv2.boundingRect(_ctr_temp)
            frame[y_temp:y_temp + h_temp, x_temp:x_temp + w_temp] = 0
        frame = shrink_image(frame)
        frame = extend_image(frame, max(frame.shape[1] // 8, 3))
